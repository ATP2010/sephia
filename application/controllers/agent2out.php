...
class Agent2out extends CI_Controller {
	...
	public function index() {
        ...
        if (!empty($cek) && $stts=='agent2out') {
            ...
            $isi['sub_judul'] = "Info";
                ...
                $this->load->view('agent2out/menu',$isi);
                $this->load->view('agent2out/home',$isi);
    ...
    public function create() {
        ...
        if (!empty($cek) && $stts=='agent2out') {
               ...
               $isi = array(
                'button' => 'Create',
                'action' => site_url('agent2out/create_action'),
                'id' => set_value('id'),
            ...
            $this->load->view('design/atas2');
            $this->load->view('agent2out/menu',$isi);
            $this->load->view('agent2out/input',$isi);
            $this->load->view('design/bawah2');
        } else { redirect('web'); }        
    }
    ...
    public function create_action(){
        ...
        foreach ($data as $key) {
            ...
            /*var_dump($_POST);*/
            redirect(site_url('agent2out/tiket_in'));
        }
        }
    }
     ...
     /*   if (!$this->input->is_ajax_request()) {
            ...
            $columns = array(
                ...
                array('db'  => '`u`.`id`', 'dt' => 11, 'field' => 'id', 'formatter' => function( $d) {
                                                                return '<a href="'.site_url('agent2out/update/' .$d).'" >update</a>';}),
    ...
    /*}*/
        ...
        public function tiket_in(){
            ...
            if (!empty($cek) && $stts=='agent2out') {
                ...
                $this->load->view('design/atas2');
                $this->load->view('agent2out/menu',$isi);
                $this->load->view('agent2out/list_data');
                $this->load->view('design/bawah2');
            }
            else
            {
    ...
    public function update($id){
        ...
        if (!empty($cek) && $stts=='agent2out') {
            ...
            if ($row) {
                $isi=array(
                    'button'=>'Update',
                    'action'=>site_url('agent2out/update_action'),
    ...
    public function delete($id){
        ...
        if ($row) {
            ...
            redirect(site_url('agent2out/list_dt'));
        } else {
     ...
     public function create_hc(){
        ...
        if (!empty($cek) && $stts=='agent2out') {
               ...
               $isi = array(
                'button' => 'Create',
                'action' => site_url('agent2out/create_hc_action'),
                'id' => set_value('id'),
            ...
            $this->load->view('design/atas2');
            $this->load->view('agent2out/menu',$isi);
            $this->load->view('agent2out/input_hc',$isi);
            $this->load->view('design/bawah2');
        } else { redirect('web'); }
    }
    ...
    public function create_hc_action(){
        ...
        foreach ($data as $key) {
            ...
            redirect(site_url('agent2out/create_hc'));
        }
        }
    }
    ...
    public function list_hc(){
         $cek = $this->session->userdata('logged_in');
            ...
            if (!empty($cek) && $stts=='agent2out') {
                ...
                $this->load->view('design/atas2');
                $this->load->view('agent2out/menu',$isi);
                $this->load->view('agent2out/list_hc');
                $this->load->view('design/bawah2');
            }
            else
            {
    ...
    public function update_hc($id){
        ...
        if (!empty($cek) && $stts=='agent2out') {
            ...
            if ($row) {
                $isi=array(
                    'button'=>'Update',
                    'action'=>site_url('agent2out/update_hc_action'),
                    'id' => set_value('id', $row->id),
    ...
    public function add_cb(){
    	$cek = $this->session->userdata('logged_in');
        $stts = $this->session->userdata('stts');
        $login = $this->session->userdata('user1');
        if (!empty($cek) && $stts=='agent2out') {

        	$ad = $this->db->query("SELECT * from cc147_caring_status where sta='1' order by lapul DESC")->row();
        	
        	if (empty($ad)) {
        		echo '<script type="text/javascript">alert("Data Tiket belum ada !"); window.location="index";</script>';
        	}else{

        	$nt = $ad->no_tiket;
        	$lg = $login;
        	$qa = $this->db->query("UPDATE cc147_caring_status set sta ='2',login='".$lg."' where no_tiket='".$nt."'");
			$isi['dt'] = $ad;
        	$isi['judul'] = 'Form input';
            $isi['sub_judul'] = "Tiket C4";
            $this->load->view('design/atas2');
            $this->load->view('agent2out/menu',$isi);
            $this->load->view('agent2out/input_c4',$isi);
            $this->load->view('design/bawah2');
        	}
        }
    }

    public function act_inputc4(){
    	$sta = $this->input->post('sta');
    	$ket = $this->input->post('ket');
    	$nt  = $this->input->post('no_tiket');
    	$lg  = $this->input->post('logid');

    	$qapdet = $this->db->query("UPDATE cc147_caring_status set sta='".$sta."', ket='".$ket."', login='".$lg."', tgl_call=NOW() where no_tiket='".$nt."' ");
    	redirect('agent2out/list_cb','refresh');
    }
    ...
    public function list_cb(){
            ...
            if (!empty($cek) && $stts=='agent2out') {
                ...
                $this->load->view('design/atas2');
                $this->load->view('agent2out/menu',$isi);
                $this->load->view('agent2out/list_cb');
                $this->load->view('design/bawah2');
            }
            else
            {
    ...
    public function ajax_data_cb(){
     ...
     /*   if (!$this->input->is_ajax_request()) {
            ...
            $columns = array(
                ...
                array('db' => '`a`.`ket`', 'dt' => 3, 'field' => 'ket'),
                array('db' => '`a`.`tgl_call`', 'dt' => 4, 'field' => 'tgl_call'),
                array('db' => '`a`.`login`', 'dt' => 5, 'field' => 'login'),
                array('db'  => '`a`.`no_tiket`', 'dt' => 6, 'field' => 'no_tiket', 'formatter' => function( $d) {
